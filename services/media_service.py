...
class MediaService:
    ...
    def download_video_to_temp(self, gcs_uri: str) -> Optional[str]:
        ...
        try:
            ...
            # Download video directly (no conversion)
            temp_download = tempfile.NamedTemporaryFile(suffix=".mp4", delete=False)
            print(f"Downloading video from GCS...")
            blob.download_to_filename(temp_download.name)
            print(f"✓ Video downloaded: {temp_download.name} ({os.path.getsize(temp_download.name)} bytes)")
            
            # Track for cleanup and return original video
